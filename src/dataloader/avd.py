...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, transform=None, is_for_online_eval=False):
        ...
        self.rgb_dir = args.rgb_dir
        self.depth_dir = args.depth_dir

        img_path = sorted(glob.glob(os.path.join(self.rgb_dir,"*.jpg")))
        self.frame_ids = [os.path.basename(p)[:13] for p in img_path]
        self.frame_ids = sorted(set(self.frame_ids))
    ...
    def __getitem__(self, idx):
        ...
        rgb_file = os.path.join(self.rgb_dir,f"{frame_id}01.jpg")
        dep_file = os.path.join(self.depth_dir,f"{frame_id}03.png")
        # print(rgb_file)
        image = np.array(Image.open(rgb_file).convert('RGB'))
        depth_gt = np.array(Image.open(dep_file)).astype(np.float32) / 1000
        
        image     = cv2.resize(image,  (TARGET_W, TARGET_H), interpolation=cv2.INTER_LINEAR)
        depth_gt  = cv2.resize(depth_gt, (TARGET_W, TARGET_H), interpolation=cv2.INTER_NEAREST)
        ...
        if self.mode == 'train':
            ...
            # fname = rgb_file
            image_path = rgb_file
            image_folder = "D:/Praise/deltar/room_03"

...
from src.dataloader.avd import AVD
...
def predict_tta(model, input_data, args):
    print("hist_data",np.shape(input_data['additional']['hist_data']))
...
def eval(model, test_loader, args, device):
    ...
    with torch.no_grad():
        ...
        for index,batch in enumerate(tqdm(test_loader)):
            ...
            final = predict_tta(model, input_data, args).squeeze().cpu().numpy()
            gt_np  = gt.squeeze().cpu().numpy()

            # debug valid pixels
            valid_mask = np.logical_and(gt_np > args.min_depth, gt_np < args.max_depth)
            print(f"Batch {index}: {valid_mask.sum()}/{valid_mask.size} valid pixels")

            # only update if we have any
            if valid_mask.any():
                errs = compute_errors(gt_np[valid_mask], final[valid_mask])
                print("  errs:", errs)
                metrics.update(errs)
...
if __name__ == '__main__':
    ...
    if args.dataset == "avd" and args.rgb_dir is not None:
    # test_loader = ZJUL5(args, 'online_eval').data
        avd = AVD(args, mode='online_eval')
        test_loader = avd.data
